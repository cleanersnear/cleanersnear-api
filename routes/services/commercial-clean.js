...
const commercialCleanHandler = {
    processBooking: async (req, res) => {
        try {
            ...
            console.log('Starting commercial cleaning booking process...');
            ...
            // 3. Create commercial service record
            const { data: serviceData, error: serviceError } = await supabase
                ...
                .insert({
                    ...
                    company_details: serviceDetails.companyDetails,
                    frequency: serviceDetails.frequency,
                    regular_frequency: serviceDetails.regularFrequency,
                    contact_info: serviceDetails.contact,
                    staff_required: serviceDetails.staffRequired,
                    hours_per_visit: serviceDetails.hoursPerVisit,
                    operating_hours: serviceDetails.operatingHours,
            ...
            const response = {
                success: true,
                message: 'Commercial cleaning booking created successfully',
                data: {
            ...
            console.error('Commercial cleaning error:', error);
            res.status(500).json({
                success: false,
                message: 'Failed to process commercial cleaning booking',
                error: error.message
            });
        }
    }
...
router.post('/submit', commercialCleanHandler.processBooking);
...
async function generateBookingNumber() {
    try {
        ...
        return `COM-${nextNumber.toString().padStart(4, '0')}`;
    } catch (error) {
