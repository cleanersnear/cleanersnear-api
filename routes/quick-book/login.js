...
const checkAuth = async (req, res, next) => {
  ...
  try {
    const { data: { user }, error } = await supabaseOld.auth.getUser(token);
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    const { data: authData, error: authError } = await supabaseOld.auth.signInWithPassword({
      email,
      password,
    });
    ...
    // Fetch customer data
    const { data: customerData, error: customerError } = await supabaseOld
...
router.post('/logout', checkAuth, async (req, res) => {
  try {
    const { error } = await supabaseOld.auth.signOut();
    if (error) throw error;
